...
function renderField(key, marker, defaultValue = "", path = "") {
  ...
  switch (marker.type) {
    ...
    case "select":
      ...
      const optionsHtml = marker.options.map(
        (opt) => `<option value="${escapeHtml$1(opt)}">`
      ).join("");
      return `
        <div class="promptcanvas-field">
    ...
    case "static":
      return `
        <div class="promptcanvas-field">
          ...
          <div class="promptcanvas-static" data-path="${path}${key}" data-static="true">${escapeHtml$1(marker.value)}</div>
        </div>
      `;
    default:
      return "";
...
function setupEventListeners(overlay, template) {
  ...
  overlay.addEventListener("click", (e) => {
    var _a;
    const addTo = e.target.dataset.addTo || ((_a = e.target.closest("[data-add-to]")) == null ? void 0 : _a.dataset.addTo);
    if (addTo) {
...
function isTextInput(element) {
  ...
  if (tagName === "input") {
    const type = (_b = element.type) == null ? void 0 : _b.toLowerCase();
